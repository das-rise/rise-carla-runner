#!/usr/bin/env bash
# Run a GbgSaroRound Digital-Twin world simulation.
#
# Usage:
#   bash run_GbgSaroRound_DT.sh [OPTIONS]
#
# Options:
...
#   bash run_GbgSaroRound_DT.sh --agent --display_camera ego_dashcam
#
#   # PCLA agent + teleport NPCs:
#   bash run_GbgSaroRound_DT.sh --agent --npcs --npc_movement teleport
#
#   # PCLA agent + behavior NPCs (cautious):
#   bash run_GbgSaroRound_DT.sh
#
#   # Behavior NPCs only, no agent:
#   bash run_GbgSaroRound_DT.sh --npcs --npc_movement behavior_agent --display_camera ego_overhead
...
TRAJ_FILES="scenes/roundabout-saro/npc_test_routes/*Car.csv"
...
cleanup() {
  local exit_code=$?
  set +e
  if [[ -n "${RUN_PYTHON_PID:-}" ]] && kill -0 "$RUN_PYTHON_PID" 2>/dev/null; then
    kill "$RUN_PYTHON_PID" 2>/dev/null || true
    wait "$RUN_PYTHON_PID" 2>/dev/null || true
  fi
  carla_docker_stop >/dev/null 2>&1 || true
  exit "$exit_code"
}
